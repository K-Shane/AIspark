...
#     # Add the following line to set dataset_path
#     dataset_path = os.path.join(CFG.path, 'aif', '찬형', '배포용_V1', 'dataset')
...
def main():
    ...
    print(f"your absolute path: {CFG.path}")
    print(f"your Dataset absolute path: {dataset_path}")

    makedirs(os.path.join(CFG.path, 'TRAIN_DATA'))
    makedirs(os.path.join(CFG.path, 'TEST_DATA'))

    concat_train(CFG.path, dataset_path)
    concat_test(CFG.path, dataset_path)
    print("Exit the program.")
